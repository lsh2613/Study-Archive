...
## 6. 그래서 어떤 알고리즘이 최곤데?
암만 봐도 마지막에 나온 ZGC가 가장 성능이 좋아보인다. 
하지만 `Java 11`부터 `Default GC` 쭉 `G1 GC`이다.

**그럼 G1 GC가 가장 좋은 건가?**<br>
선택에 절대적인 건 없다. 각 GC 마다 특징이 있을 뿐이다

`ZGC`는 `극단적인 저지연`(Low-Latency)을 목적으로 하고 `G1 GC`는 `예측 가능한 응답시`간을 제공하기 위한 목적으로 설계되었다고 한다

따라서 `ZGC`는 초저지연이 중요한 `실시간 처리 시스템`, `대규모 시스템`, `금융 시스템` 등에 좀 더 적합하고, 
`G1 GC`는 `일정한 응답 시간`을 유지하며 `예측 가능한 응답시간`이 중요한 애플리케이션에 사용된다고 한다.

이러한 이유로 봤을 때, Java는 대부분 Web을 구현하기 위해 사용되고(아마?), 대부분의 웹은 초저지연 보다는 예측 가능한 응답 시간을 제공을 목표로 하기 떄문이지 않을까 싶다
